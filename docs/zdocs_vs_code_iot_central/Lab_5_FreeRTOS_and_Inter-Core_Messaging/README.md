...
## Windows users: Enable Real-Time Core Development
...
## Ubuntu users: Enable Real-Time Core Debugging

The Azure Sphere SDK for Linux does not include the GNU compiler that is required for the ARM Cortex-M4 processors on the real-time capable cores. You can find the GNU embedded toolchain for ARM that includes the compiler on the [Arm developer website](https://developer.arm.com/tools-and-software/open-source-software/developer-tools/gnu-toolchain/gnu-rm/downloads). Download the package appropriate for your version of Linux and consult the readme.txt for the appropriate installation instructions.
...
2. Under **Azure Sphere: Arm Gnu Path** enter the path to the GNU Arm Embedded Toolchain install directory. By default, the toolchains are installed in /opt/azurespheresdk/Sysroots/sysroot-version/tools, as described in [Install the Azure Sphere SDK on Linux](https://docs.microsoft.com/en-us/azure-sphere/install/install-sdk-linux?WT.mc_id=julyot-azd-dglover).

Download the ARM Toolchain and install as follows:

  ```bash
  sudo tar -xjvf gcc-arm-none-... -C /opt
  ```

  Add to path or to ~/.bashrc

  ```bash
  export PATH=$PATH:/opt/gcc-arm-none-.../bin/
  ```
...
3. Open the **Lab_4_FreeRTOS_Integration** folder
