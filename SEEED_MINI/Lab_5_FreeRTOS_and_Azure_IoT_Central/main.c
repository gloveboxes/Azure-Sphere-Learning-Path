...
static DeviceTwinBinding light = {
	.peripheral = {
		.fd = -1, .pin = LIGHT_PIN, .direction = OUTPUT, .initialState = GPIO_Value_Low, .invertPin = false, .initialise = OpenPeripheral, .name = "led1" },
	.twinProperty = "led1",
	.twinType = TYPE_BOOL,
	.handler = DeviceTwinHandler
};
...
static Peripheral builtinLed = {
	.fd = -1, .pin = LED_2, .direction = OUTPUT, .initialState = GPIO_Value_Low, .invertPin = false, .initialise = OpenPeripheral, .name = "SendStatus"
};
